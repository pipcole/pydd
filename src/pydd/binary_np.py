...
def _Phi_to_c_indef_a(f, params: AccretionDisk):#2pi intdf f dt/df between fc and f
    ...
    zf = 4 * np.sqrt(2/3) * f**(8/3) * G * get_m_1(params.M_chirp, params.q)**3\
     * np.pi**(8/3)/(5 * C**4 * (G * totm**4)**(1/3) * params.Mach**2 * params.Sigma0)
...
#     *jnp.pi**(8/3)/(5 * C**4 * (G * totm**4)**(1/3) *params.Mach**2 * params.Sigma0)
    ...
    return (
        -2 * np.pi* (C * get_m_1(params.M_chirp, params.q)**2 *(G * totm)**(
 1/3) * (-f * hypgeomacc(zf))/(12 * np.sqrt(6) * G * get_m_2(params.M_chirp, params.q) * (G * totm**4)**(
...
def _t_to_c_indef_a(f, params: AccretionDisk):# -int dfdt/df betweem fc and f
    totm=get_m_1(params.M_chirp,params.q) + get_m_2(params.M_chirp,params.q)
    return (-
        (1/(96 * np.sqrt(6) * get_m_2(params.M_chirp, params.q) * (G * totm**4)**(
 1/3) * params.Mach**2 * params.Sigma0) * C * get_m_1(params.M_chirp, params.q)**2 * (totm/G**2)**(
 1/3) * (-8 * np.log(f)+
   3 *np.log(
     8 *f**(8/3) * G * get_m_1(params.M_chirp, params.q)**3 * np.pi**(8/3) -
      5 * np.sqrt(6) * C**4 * (G * totm**4)**(1/3) * params.Mach**2 * params.Sigma0))
    )
    )
...
def d2Phi_dt2_a(f, params: AccretionDisk):
    ...
    return (-((24 * f**3 * get_m_2(params.M_chirp, params.q) * ((G * totm)/f**2)**(2/3)\
         * np.pi* (-8 * f**2 * G * get_m_1(params.M_chirp, params.q)**3 * np.pi**(8/3) +5\
          * np.sqrt(6) * C**3 * totm * np.sqrt(G *totm)* ((G *totm)/f**2)**(1/6)\
           * np.sqrt(C**2/(f**2 *((G *totm)/f**2)**(2/3))) * params.Mach**2 * params.Sigma0)/\
...
#    return t_interp_ga(get_m_1(params.M_chirp, params.q)/MSUN)(f)
    ...
    return np.interp(f,gatom_results[2],-gatom_results[0]*(60*60*24*365.25))
