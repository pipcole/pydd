...
def Rbound(n, l, r, rs, alpha): #Wavefunction of the bound states Eq. (2.5)
    rbohr = rs / (2*alpha**2)
    return sqrt((4*factorial(n-l-1))/(rbohr**3*n**4*factorial(n+l))) * exp(-r/(n*rbohr)) * (2*r/(n*rbohr))**l * sc.genlaguerre(n-l-1,2*l+1)(2*r/(n*rbohr))

def dqOverdt(q, rs, r, epsilon, n, l, m, alpha, chi): #Mass accretion secondary object Eq.(5.1)
    return 4*pi*q**2*rs**2 * epsilon * Rbound(n,l,r,rs,alpha)**2 * abs(sc.sph_harm(m,l,0,pi/2))**2
...
def dtOverdr(q, dqOverdt, rs, r, m, Omega0, epsilon, alpha, ion_energy): #Eq.(5.6)
    ...
    dEaccretiondt = sqrt((1+q)*rs/(2*r**3)) * ( ((q+2)/(2*(1+q)**1.5)) * sqrt(rs*r/2) - m*rs/(2*alpha) ) * dqOverdt
